...
public class ToDoList {
	...
	public static synchronized boolean removeToDo(String name) throws Exception {
		ToDoList toDoList;
		Gson gson = new Gson();
		try {
			toDoList = gson.fromJson(new FileReader(DEFAULT_FILE_NAME), ToDoList.class);
		...
		for(Iterator<ToDo> it = toDoList.toDoList.iterator(); it.hasNext(); ){
			if(it.next().getTask().equals(name)){
				it.remove();
				remove = true;
			}
		}
